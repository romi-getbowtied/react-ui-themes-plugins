<?php
/**
 * Hero Parallax Enhanced Component
 * 
...
/**
 * Get hero parallax products data
 * Can be filtered using 'tw_hero_products' filter
 * 
...
function tw_render_hero_parallax($items_per_row = null) {
	...
	if ($items_per_row === null) {
		$items_per_row = apply_filters('tw_hero_items_per_row', 5);
	}
	$products = tw_get_hero_products();
	$total_products = count($products);
	
	// Dynamically split products into rows
	$rows = [];
	for ($i = 0; $i < $total_products; $i += $items_per_row) {
		$rows[] = array_slice($products, $i, $items_per_row);
	}
	
	// Calculate section height based on number of rows (approximately 100vh per row)
	$section_height = max(300, count($rows) * 100); // Minimum 300vh, add 100vh per row
	?>
	<section 
	...
	>
		...
		<!-- Products Grid - Server-side rendered for SEO -->
		<div class="hero-parallax-container">
			<?php foreach ($rows as $row_index => $row_products) : 
				$row_number = $row_index + 1;
				// Alternate row direction: odd rows = reverse, even rows = normal
			...
			?>
				<div class="<?php echo esc_attr($row_class); ?> mb-20 hero-parallax-row" data-row="<?php echo esc_attr($row_number); ?>" data-direction="<?php echo $is_reverse ? 'reverse' : 'normal'; ?>">
					<?php foreach ($row_products as $product_index => $product) : 
						// Eager load first 2 images of first row, lazy load the rest
