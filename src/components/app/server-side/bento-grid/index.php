...
if (!class_exists('TW_Bento_Grid')) {
	class TW_Bento_Grid
	{
		...
		public static function render($items = null, $className = '')
		{
			$items = $items ?? self::get_items();
			$grid_classes = 'mx-auto grid max-w-7xl grid-cols-1 gap-4 md:auto-rows-[18rem] md:grid-cols-3' . ($className ? ' ' . esc_attr($className) : '');
			?>
	<section 
	...
	>
		...
		<div class="<?php echo esc_attr($grid_classes); ?>">
			...
			foreach ($items as $index => $item):
				$item_classes = 'group/bento shadow-input row-span-1 flex flex-col justify-between space-y-4 rounded-xl border border-neutral-200 bg-white p-4 transition duration-200 hover:shadow-xl dark:border-white/[0.2] dark:bg-black dark:shadow-none' . (!empty($item['className']) ? ' ' . esc_attr($item['className']) : '');
				...
				<article 
					...
					itemprop="itemListElement"
							itemid="#bento-item-<?php echo esc_attr($index + 1); ?>"
				>
